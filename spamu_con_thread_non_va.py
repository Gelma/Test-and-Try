...
if True: # variabili globali
	...
	pausa_tra_thread				= 5 # secondi tra i lanci di thread di controllo
	tempo_minimo_per_i_controlli	= 60 # numero minino di secondi concessi per i controlli
...
class LUG(persistent.Persistent):
	...
	def controllo_homepage(self):
		...
		try:
			Termini_Attuali = set(self._v_browser.open(self.url).read().split()) # Estrapolo parole
		except:
		...
		try: # per evitare segnalazione su un Lug nuovo, se self.Termini_Precedenti non esiste
			valore_magico = \
			  float(len(self.Termini_Precedenti.intersection(self_v_Termini_Attuali))*1.0/len(self.Termini_Precedenti.union(self._v_Termini_Attuali)))
		except:
...
if __name__ == "__main__":
	...
	for id in zodb.keys(): # lancio ogni thread a distanza di n secondi
		multiprocessing.Process(target=zodb[id].controlli, name=id, args=(coda_risultati,)).start()
		time.sleep(pausa_tra_thread)

	logga('attendo ',tempo_minimo_per_i_controlli,' secondi per permettere a tutti di concludere i controlli')
	time.sleep(tempo_minimo_per_i_controlli)
